<?php
	...
	$query = "SELECT *, DATE_FORMAT(added, '%a, %d %b %Y %T') as pub_date FROM cashbackengine_retailers WHERE status='active' ORDER BY added DESC LIMIT 10000";
	...
	if ($total > 0)
	{
		...
		echo '<image>';
		echo '<url>'.SITE_URL.'images/PandaCashBack.png</url>';
		echo '<title>'.SITE_TITLE.'</title>';
		echo '<link>'.SITE_URL.'</link>';
		echo '</image>';
